...
export function TransactionModal({ transaction, holdings, onClose, onSaved }: { transaction: Transaction | null; holdings: Holding[]; onClose: () => void; onSaved: () => void }) {
  ...
  return <div className="modal-backdrop"><section className="modal"><div className="modal-header"><div><p className="eyebrow">{transaction ? 'EDIT TRANSACTION' : 'NEW TRANSACTION'}</p><h2>{transaction ? label(transaction.type) : 'Log a transaction'}</h2></div><button className="close" onClick={onClose}>×</button></div>
    <form onSubmit={submit}><div className="form-grid">
      ...
      <Field label={form.type === 'SELL' ? 'Proceeds' : form.type === 'REPAY' ? 'Repayment amount' : form.type === 'INTEREST' ? 'Income' : 'Amount'} required><input required type="number" min="0" step="0.01" value={form.amount} onChange={e => set('amount', e.target.value)} /></Field>
    ...
    <div className="modal-actions"><button type="button" className="outline" onClick={onClose}>Cancel</button><button className="primary" disabled={saving || !form.holdingId || !form.amount || (form.type === 'SPLIT' && !form.quantity)}>{saving ? 'Saving…' : transaction ? 'Save changes' : 'Log transaction'}</button></div>
    </form>
  </section></div>
}
